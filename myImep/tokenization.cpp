...
//        }
    ...
    AutoTokenizer::AutoTokenizer(const std::string &tokenizer_json_file){
        std::cout << "Tokenizer: Huggingface" << std::endl;

        auto start = std::chrono::high_resolution_clock::now();

        // Read blob from file.
        auto blob = LoadBytesFromFile(tokenizer_json_file);
    ...
    GPT2Tokenizer::GPT2Tokenizer(
        ...
        while (std::getline(file_merges, line)) {
            ...
            if (!line.empty()) {
                if(((wchar_t) line[0]) == L'臓') {
                    bpe_merges.push_back(line.substr(1));
                } else {
        ...
        for(int i = 0;i<10;i++) {
            std::cout << bpe_merges[i] << std::endl;
        }
